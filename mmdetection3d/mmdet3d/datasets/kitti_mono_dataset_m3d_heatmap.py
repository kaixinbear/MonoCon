    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        def _load_depth_map(idx):
            ...
            Returns:
                depth: (H, W), Depth map
            """
            depth_file = self.data_root / 'training' / 'depth_2' / ('%s.png' % idx)
            assert depth_file.exists(), str(depth_file) + ' not exists'
        ...
        depth_map = _load_depth_map(img_info['file_name'].split('/')[-1].split('.')[0])

        ann = dict(
            depth_map=depth_map,
            img_fields=['img', 'depth_map'],    # let depth_map do data-process as img
            bboxes=gt_bboxes,
            labels=gt_labels,
