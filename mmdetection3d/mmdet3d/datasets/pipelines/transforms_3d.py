...
@PIPELINES.register_module()
class DownsampleDepthMap:
    """
    Downsampling Depth map.

    Args:
        DOWNSAMPLE_FACTOR(int): downsample rate
    """

    def __init__(self, DOWNSAMPLE_FACTOR=4):
        self.depth_downsample_factor = DOWNSAMPLE_FACTOR
    ...
    def __call__(self, results):
        ...
        results['depth_map'] = transform.downscale_local_mean(
            image=results['depth_map'],
            factors=(self.depth_downsample_factor, self.depth_downsample_factor)
        )
        # print("depth", results['depth_map'].shape)
        # print("img", results['img'].shape)
        # print(results['depth_map'])
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(depth_downsample_factor={self.depth_downsample_factor})'
        return repr_str
