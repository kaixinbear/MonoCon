...
from mmdet3d.models.utils.gaussian_target_3D import bin_to_depth, gen_gaussian_depth_target                                         
...
# 把深度估计从直接估计depth改成预测一个heatmap，这里先不加pixel level的深度估计
@HEADS.register_module()
class MonoP2OHead_Wo_P(nn.Module):
    def __init__(self,
                 in_channel,
                 feat_channel,
                 num_classes,
                ...
                #  loss_depth=None,
                 ...
                 test_cfg=None,
                 init_cfg=None):
        super(MonoP2OHead_Wo_P, self).__init__()
        ...
        self.depth_heatmap_head = self._build_head(in_channel, feat_channel, depth_bins)
        ...
        self.loss_depth_heatmap = build_loss(loss_depth_heatmap)
    ...
    def init_weights(self):
        ...
        for head in [self.wh_head, self.offset_head, self.center2kpt_offset_head, self.depth_heatmap_head,
    ...
    def forward_single(self, feat):
        ...
        # 先用sigmoid， 因为生成的gt有三个1；
        depth_pred = self.depth_heatmap_head(feat).sigmoid()
        depth_pred = torch.clamp(depth_pred, min=1e-4, max=1 - 1e-4)
        ...
        return center_heatmap_pred, depth_pred, wh_pred, offset_pred, center2kpt_offset_pred, kpt_heatmap_pred, \
               kpt_heatmap_offset_pred, dim_pred, alpha_cls_pred, alpha_offset_pred
    ...
    def loss(self,
        ...
        depth_heatmap_pred = depth_heatmap_preds[0]
        wh_pred = wh_preds[0]
        ...
        depth_heatmap_target = target_result['depth_heatmap_target']
        ...
        return dict(
            ...
            loss_depth_heatmap=loss_depth_heatmap,
            loss_wh=loss_wh,
    ...
    def get_targets(self, gt_bboxes, gt_labels,
        ...
        # object depth heatmap
        depth_heatmap_target = gt_bboxes[-1].new_zeros([bs, self.depth_bins, feat_h, feat_w])
        ...
        for batch_id in range(bs):
            ...
            for j, ct in enumerate(gt_centers):
                ...
                gen_gaussian_depth_target(center_heatmap_target[batch_id, ind],
                                    [ctx_int, cty_int], radius, depth_heatmap_target[batch_id], depth[j], self.depth_bins)
        ...
        target_result = dict(
            ...
            depth_heatmap_target=depth_heatmap_target,
            wh_target=wh_target,
    ...
    def decode_heatmap(self,
        ...
        *batch_dets, ys, xs = get_topk_from_heatmap(
        ...
        depth_heatmap_pred = transpose_and_gather_feat(depth_heatmap_pred, batch_index)
        topk_depths_scores, topk_depth_inds = torch.topk(depth_heatmap_pred, k=3, dim=-1)
        tops_depths = bin_to_depth(topk_depth_inds)  #(B, N, 3)
        depth = torch.mean(tops_depths, dim=-1, keepdim=True)

        sigma = torch.mean(topk_depths_scores, dim=-1, keepdim=False)
