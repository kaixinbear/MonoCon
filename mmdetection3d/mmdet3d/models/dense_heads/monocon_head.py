...
@HEADS.register_module()
class MonoConHead_W_DepthDis(nn.Module):
    def __init__(self,
                 in_channel,
                 feat_channel,
                 num_classes,
                 ...
                 test_cfg=None,
                 init_cfg=None):
        super(MonoConHead_W_DepthDis, self).__init__()
        assert bbox3d_code_size >= 7
        self.num_classes = num_classes
        self.bbox_code_size = bbox3d_code_size
        self.pred_bbox2d = pred_bbox2d
        ...
        self.use_AN = use_AN
        self.use_depth_estimate = use_depth_estimate
        self.num_AN_affine = num_AN_affine
        self.norm = AttnBatchNorm2d if use_AN else nn.BatchNorm2d

        self.depth_dis_head = self._build_head(in_channel, feat_channel, 81) if use_depth_estimate else None
        self.depth_dis_loss = DepthLoss(**loss_depth_estimation)
    ...
    def forward_single(self, feat):
        ...
        depth_dis_pred = self.depth_dis_head(feat)

        return center_heatmap_pred, wh_pred, offset_pred, center2kpt_offset_pred, kpt_heatmap_pred, \
               kpt_heatmap_offset_pred, dim_pred, alpha_cls_pred, alpha_offset_pred, depth_pred, depth_dis_pred

    @force_fp32(apply_to=('center_heatmap_preds', 'wh_preds', 'offset_preds', 'center2kpt_offset_preds',
                          'kpt_heatmap_preds', 'kpt_heatmap_offset_preds', 'dim_preds', 'alpha_cls_preds',
                          'alpha_offset_preds', 'depth_preds', 'depth_dis_pred'))
    def loss(self,
             center_heatmap_preds,
             wh_preds,
             offset_preds,
        ...
        assert len(center_heatmap_preds) == len(wh_preds) == len(offset_preds) \
               == len(center2kpt_offset_preds) == len(kpt_heatmap_preds) == len(kpt_heatmap_offset_preds) \
               == len(dim_preds) == len(alpha_cls_preds) == len(alpha_offset_preds) == len(depth_dis_preds) == 1
        ...
        alpha_offset_pred = alpha_offset_preds[0]
        depth_pred = depth_preds[0]
        depth_dis_pred = depth_dis_preds[0]
        ...
        loss_depth_dis, _ = self.depth_dis_loss(depth_dis_pred, gt_depth_map, gt_bboxes)
        ...
        return dict(
            ...
            loss_alpha_reg=loss_alpha_reg,
            loss_depth=loss_depth,
            loss_depth_dis= loss_depth_dis['loss_depth_estimate']
        )
    ...
    def forward_train(self,
        ...
        loss_inputs = outs + (gt_bboxes, gt_labels, gt_bboxes_3d,
                              gt_labels_3d, centers2d, depths, gt_kpts_2d, gt_kpts_valid_mask,
                              img_metas, gt_depth_map, attr_labels)
        losses = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore)
